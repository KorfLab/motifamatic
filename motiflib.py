...
def states(file_gen):
    ...
    marked = []    
    for i in range(len(bool_states[0])): # iterate down row
        count = 0
        for j in range(len(bool_states)): # iterate across column
            if bool_states[j][i] == True:
                count += 1
        print(i, count)
        if count > 0.5*len(bool_states):
            marked.append("M")
        else:
            marked.append("I")

    # match or insert states of sequences
    states = []
    for row in bool_states: # iterate down row
        temp = []
        for ind_col, col in enumerate(row): # iterate across column
            if col == True and marked[ind_col] == "M":
                temp.append("M")
            elif col == False and marked[ind_col] == "M":
                temp.append("D")
            elif col == True and marked[ind_col] == "I":
                temp.append("I")
            else:
                temp.append("-")
        states.append(temp)
